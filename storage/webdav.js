...
// ... (保留 getWebdavConfig, getClient, resetClient, ensureDirectoryExists, getFolderPath) ...
...
function getClient() {
    if (!client) {
        ...
        client = createClient(webdavConfig.url, { username: webdavConfig.username, password: webdavConfig.password });
    }
    return client;
}
...
async function ensureDirectoryExists(fullPath) {
    ...
    while (creatingDirs.has(fullPath)) await new Promise(resolve => setTimeout(resolve, 50));
    ...
    try {
        ...
        for (const part of pathParts) {
            currentPath += `/${part}`;
            if (!(await client.exists(currentPath))) { try { await client.createDirectory(currentPath); } catch (e) {} }
        }
    } finally { creatingDirs.delete(fullPath); }
}
...
async function upload(fileStream, fileName, mimetype, userId, folderId, caption = '', existingItem = null) {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            if (fileStream && typeof fileStream.on === 'function') fileStream.on('error', err => reject(new Error(`Stream Error: ${err.message}`)));
            let options = { overwrite: true };
            if (fileStream && fileStream.path && typeof fileStream.path === 'string') {
                try { const fsStats = fs.statSync(fileStream.path); options.contentLength = fsStats.size; } catch (e) {}
            }
            const success = await client.putFileContents(remotePath, fileStream, options);
            if (!success) throw new Error('WebDAV putFileContents 返回 false');
            const stats = await client.stat(remotePath);
            if (stats.size === 0) throw new Error('上传验证失败: 远端文件大小为 0 字节');
            if (existingItem) {
                await data.updateFile(existingItem.message_id, { mimetype: mimetype, file_id: remotePath, size: stats.size, date: Date.now() }, userId);
                resolve({ success: true, message: '覆盖成功', fileId: existingItem.message_id });
            } else {
                const messageId = BigInt(Date.now()) * 1000000n + BigInt(crypto.randomInt(1000000));
                const dbResult = await data.addFile({
                    message_id: messageId, fileName, mimetype, size: stats.size, file_id: remotePath, date: Date.now(),
            ...
            if (client && remotePath) try { await client.deleteFile(remotePath); } catch (e) { }
            reject(error);
        }
    });
}
...
async function remove(files, folders, userId) {
    ...
    folders.forEach(folder => { // 软删除后，物理删除逻辑可能只在永久删除时触发
    ...
    for (const item of allItemsToDelete) {
        try { await client.deleteFile(item.path); } 
        catch (error) { if (!(error.response && error.response.status === 404)) { results.errors.push(`删除失败: ${error.message}`); results.success = false; } }
    }
    return results;
}
...
async function createDirectory(fullPath) {
    ...
    try {
        ...
        return true;
    } catch (e) { if (e.response && (e.response.status === 405 || e.response.status === 501)) return true; throw new Error(`建立目录失败: ${e.message}`); }
}
