    ...
    try {
        ...
        for (const itemId of itemIds) {
            try {
                ...
                if (items.length === 0) {
                    // 这就是你遇到的情况：ID 为 6 的项目在这里就找不到了
                    console.log(`移动操作已跳过不存在的项目 ID: ${itemId}`);
                    skippedCount++;
                    continue; // 跳过不存在的项目，继续处理下一个
                }
...
app.get('/share/download/:folderToken/:fileId', async (req, res) => {
    try {
        const { folderToken, fileId } = req.body;
        const fileInfo = await data.findFileInSharedFolder(parseInt(fileId, 10), folderToken);
...
app.listen(PORT, () => console.log(`✅ 伺服器已在 http://localhost:${PORT} 上运行`));require('dotenv').config();
const express = require('express');
const session = require('express-session');
const multer = require('multer');
const path = require('path');
